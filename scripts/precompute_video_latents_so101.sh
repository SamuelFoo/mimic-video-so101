#!/bin/bash
#
# Precompute VAE latents for mimic-video-so101 training.
#
# Run this once after converting your zarr dataset to safetensors
# (scripts/convert_zarr_to_safetensors.py).  Outputs go to
# ${MIMIC_VIDEO_DATASET_DIR}/.latent_cache/{train,val}_<stats_id>.pt
# and are auto-detected by MimicDataset on the next training run.
#
# If the cache files already exist for the current stats_id the script
# skips re-encoding.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
MODEL_DIR="${REPO_ROOT}/mimic-video-so101/model"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${REPO_ROOT}/mimic-video/model/checkpoints}"

EXPERIMENT="${EXPERIMENT:-w2a_lerobot_iter_000001410_fused_lr1.000e-04_layer20_bsz128}"

# Must match whatever MIMIC_VIDEO_DATASET_DIR you pass to train — MimicDataset
# globs **/*.safetensors under this dir.
export MIMIC_VIDEO_DATASET_DIR="${MIMIC_VIDEO_DATASET_DIR:-${REPO_ROOT}/staging/mimic-video-st}"
export LATENT_PRECOMPUTE_BATCH_SIZE="${LATENT_PRECOMPUTE_BATCH_SIZE:-2}"
export LATENT_PRECOMPUTE_NUM_WORKERS="${LATENT_PRECOMPUTE_NUM_WORKERS:-12}"

# Optional: path to an existing .latent_cache directory from a previous run
# (e.g. staging/mimic-video/.latent_cache from the zarr-based dataset).
# If a .pt file there has the same sample count, it will be copied instead of
# re-encoding — saves the full VAE precompute when only the format changed.
export LATENT_CACHE_SOURCE_DIR="${LATENT_CACHE_SOURCE_DIR:-${REPO_ROOT}/staging/mimic-video/.latent_cache}"

export XDG_CACHE_HOME="${XDG_CACHE_HOME:-${HOME}/.cache}"
export HF_HOME="${HF_HOME:-${XDG_CACHE_HOME}/huggingface}"
export TORCH_HOME="${TORCH_HOME:-${XDG_CACHE_HOME}/torch}"
mkdir -p "${HF_HOME}" "${TORCH_HOME}"

# The venv lives in mimic-video/model — so101 shares it.
VENV_DIR="${REPO_ROOT}/mimic-video/model/.venv"
source "${VENV_DIR}/bin/activate"

export PATH="/sbin:/usr/sbin:${PATH}"
export CUDA_HOME="${VENV_DIR}/lib/python3.10/site-packages/nvidia/cuda_nvrtc"
export CUDA_PATH="${CUDA_HOME}"
_CUDA_RUNTIME_LIB="${VENV_DIR}/lib/python3.10/site-packages/nvidia/cuda_runtime/lib"
if [[ -f "${_CUDA_RUNTIME_LIB}/libcudart.so.12" && ! -e "${_CUDA_RUNTIME_LIB}/libcudart.so" ]]; then
    ln -sf libcudart.so.12 "${_CUDA_RUNTIME_LIB}/libcudart.so"
fi
export LD_LIBRARY_PATH="${_CUDA_RUNTIME_LIB}:${VENV_DIR}/lib/python3.10/site-packages/nvidia/cuda_nvrtc/lib:${LD_LIBRARY_PATH:-}"
export LD_LIBRARY_PATH="${VENV_DIR}/lib/python3.10/site-packages/nvidia/cudnn/lib:${LD_LIBRARY_PATH}"

export COSMOS_PREDICT2_ARGS="${COSMOS_PREDICT2_ARGS:---checkpoints ${CHECKPOINT_DIR}}"
export CUDA_DEVICE_MAX_CONNECTIONS=1
export NVTE_FUSED_ATTN=0
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

cd "${MODEL_DIR}"

NPROC_PER_NODE="${NPROC_PER_NODE:-$(nvidia-smi -L 2>/dev/null | wc -l)}"
NPROC_PER_NODE="${NPROC_PER_NODE:-1}"

echo "=== Precompute VAE latents (mimic-video-so101) ==="
echo "Node:        $(hostname)"
echo "Experiment:  ${EXPERIMENT}"
echo "Dataset:     ${MIMIC_VIDEO_DATASET_DIR}"
echo "GPUs:        ${NPROC_PER_NODE}"
echo

torchrun \
    --nnodes=1 \
    --nproc_per_node="${NPROC_PER_NODE}" \
    --rdzv_id="precompute_$$" \
    --rdzv_backend=c10d \
    --rdzv_endpoint="localhost:12342" \
    -m scripts.precompute_video_latents \
    --config=cosmos_predict2/configs/config.py \
    -- experiment="${EXPERIMENT}"

echo "=== Done. Caches written under ${MIMIC_VIDEO_DATASET_DIR}/.latent_cache/ ==="
